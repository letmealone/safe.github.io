...
**함께 보는 결과물:** [클릭 가능한 전체 시안](workflow.html) · [24단계 화면과 개편 이유](screen_gallery.html)
...
2. **건축물과 점검건의 연결**: 지도·대장·현장사진·BIM·보고서로 이동해도 같은 건축물, 동, 점검유형, 회차를 유지한다.
...
```text
공무원 업무 공간
  내 업무                  관할·담당 범위 / 기한·확인할 건 / 최근 열람
  건축물·점검 현황           정기점검 / 소규모노후 / 실태조사 등 업무별 목록
  점검결과·보고서            기존 결과 / 검토할 자료 / 회차 비교
  관할 현황·대상 선별         지도 + 후보 목록 + 선별 근거
  관련 행정업무              녹색건축물 지원 / 위반건축물 등 담당 권한별

전문기관 업무 공간
  내 기관 업무               수행 중인 점검 / 작성·보완 / 현장 예정
  점검·진단                  업무유형별 담당 건 / 현장 준비 / 기록
  점검결과·보고서             누락 항목 / 결과 확인 / 기존 출력
  건축물·점검 이력            공유 가능한 자료 / 지난 회차 비교

공통
  건축물 통합검색 / 현재 건축물·동·업무 건 / 자료·도움말

실증·데이터 운영 공간         해당 권한 사용자에게 제공
  sBIM·레이어·자료 관리 / AI 실행·검증 / API / 계정·권한 / 실증 관리

대민 서비스                 별도의 이용 목적에 맞는 진입
  민간 자가점검 / 수선 정보
```
...
[클릭 가능한 시안 시작](workflow.html) · [24단계 화면을 설명과 함께 읽기](screen_gallery.html)

단순한 메인화면 이미지에 그치지 않고, 대표 업무 한 건을 기관과 공무원이 이어서 처리하는 흐름을 구현했다. 이름·주소·날짜·근거 도식·보고서 내용은 모두 가상이며 실제 플랫폼과 연결되지 않는다.

### 권장 체험 순서

1. 로그인 화면에서 시안 검토용 **전문기관 담당자**를 선택하고 **로그인 후 메인화면 보기**를 누른다. 실제 계정 입력은 필요 없다.
...
9. 공무원 메뉴의 **관할 현황·대상 선별**에서 지도 후보 → 근거 → 기존 점검건 → 결과·후속 확인으로 이어지는 별도 경로를 살펴본다.
...
**상단 ‘이 화면을 바꾼 이유’**에서는 현재 관찰 → 불편한 이유 → 대안 → 기대 효과를 읽을 수 있다. **‘전체 화면 순서’**에서는 24단계의 가상 상태를 바로 열 수 있다. 하단의 역할 전환은 시안 체험 도구이며, 실제 제품에서 일반 사용자가 다른 기관·공무원 권한을 자유롭게 바꾸게 하자는 뜻이 아니다.
...
24단계의 화면 렌더링, 1440px 화면의 문서 가로 넘침, 1366×768 업무 목록과 390px 현장 목록의 화면 폭도 확인했다. 좁은 화면의 상세 표는 내부 가로 스크롤을 사용한다. 이 검증은 **가상 시안의 동작 확인**이며 현행 사이트의 저장·제출·권한·모바일 실기기 검증을 대신하지 않는다.

시안의 내용은 메모리에만 존재하며 새로고침하면 초기 예시 상태로 돌아간다. 실제 웹사이트로의 데이터 전송·저장은 없다.
